...
class Table(Settings.Base):
    ...
    retryTime = sqlalchemy.Column(sqlalchemy.Integer, nullable=False, index=True)
    startTime = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, index=True)
    endTime = sqlalchemy.Column(sqlalchemy.DateTime, nullable=True, index=True)
